...
int psm_read(psm_t *p, char *filename)
{
	...
	fp = fopen(filename, "r");
	if(fp == NULL) {
		printf("[error] Can't open PSM file %s!\n", filename);
		return 0;
	}
    ...
    for(i = 0; i < PSM_N_STATES; i++) {
        for(j = 0; j < PSM_N_STATES; j++) {
			if(i != j && psm_tran_allowed(p, i, j)) {
				printf("[psm] %s -> %s transition: energy  = %.4fmJ, time = %.4fms\n",
...
// wrappers around PSM's data structure to print specific information
...
{
    return psm.power[curr] * time * (PSM_POWER_UNIT * PSM_TIME_UNIT) / PSM_ENERGY_UNIT;
}
